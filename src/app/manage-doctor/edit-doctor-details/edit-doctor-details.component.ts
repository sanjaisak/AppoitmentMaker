...
export class EditDoctorDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.previousDOB =
      this.selectedDoctorDetails.dob.getUTCFullYear() +
      "-" +
      monthDOB.slice(-2) +
      "-" +
      dateDOB.slice(-2);
    ...
    this.doctorInfo = this.formBuilder.group({
      doctorName: [
        this.selectedDoctorDetails.name,
        [Validators.required, Validators.maxLength(20)],
      ],
      speciality: [
        this.selectedDoctorDetails.speciality,
        [Validators.required],
      ],
      dob: [this.previousDOB, [Validators.required]],
      phoneNumber: [
        this.selectedDoctorDetails.phoneNumber,
  ...
  public required(input: String): boolean {
    return (
      this.doctorInfo.get(input).errors?.required &&
